...
interface DeckDao {
    ...
    @Query("SELECT id, set_name, aspect_id, aspect_short_name, cost, real_image_src, name, " +
            ...
            "approach_exploration FROM card WHERE type_id = 'role' AND set_id = :specialty"
    )
    ...
    @Query("SELECT id, set_name, aspect_id, aspect_short_name, cost, real_image_src, name, " +
            ...
            "approach_connection, approach_reason, approach_conflict, approach_exploration FROM card " +
    ...
    @Query("SELECT card.id, card.set_name, card.aspect_id, card.aspect_short_name, card.cost, " +
            "card.real_image_src, card.name, card.type_name, card.traits, card.real_traits, card.level, " +
            "card.set_id, card.set_type_id, card.deck_limit, card.approach_connection, " +
            "card.approach_reason, card.approach_conflict, card.approach_exploration " +
            "FROM card JOIN card_fts ON (card.id = card_fts.id) WHERE set_id = 'personality' " +
    ...
    @Query("SELECT card.id, card.set_name, card.aspect_id, card.aspect_short_name, card.cost, " +
            ...
            "card.set_id, card.set_type_id, card.deck_limit, card.approach_connection, " +
            "card.approach_reason, card.approach_conflict, card.approach_exploration " +
            "FROM card JOIN card_fts ON (card.id = card_fts.id) WHERE set_id = :background " +
            "AND set_type_id = 'background' AND type_id != 'role' AND (card_fts MATCH :query) " +
            "ORDER BY aspect_id, set_position")
    fun searchBackgroundCards(query: String, background: String): PagingSource<Int, CardDeckListItemProjection>

    @Query("SELECT card.id, card.set_name, card.aspect_id, card.aspect_short_name, card.cost, " +
            "card.real_image_src, card.name, card.type_name, card.traits, card.real_traits, card.level, " +
            "card.set_id, card.set_type_id, card.deck_limit, card.approach_connection, card.approach_reason, " +
            "card.approach_conflict, card.approach_exploration FROM card JOIN card_fts ON (card.id = card_fts.id)" +
            "WHERE set_id = :specialty AND set_type_id = 'specialty' AND type_id != 'role' " +
            "AND (card_fts MATCH :query) ORDER BY aspect_id, set_position")
