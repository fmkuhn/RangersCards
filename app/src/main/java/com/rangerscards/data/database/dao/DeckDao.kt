...
interface DeckDao {
    ...
    @Query("SELECT id, set_name, aspect_id, aspect_short_name, cost, real_image_src, name, " +
            "type_name, traits, real_traits, level, set_id, set_type_id, deck_limit FROM card " +
    ...
    @Query("SELECT card.id, card.set_name, card.aspect_id, card.aspect_short_name, card.cost, " +
            ...
            "card.set_id, card.set_type_id, card.deck_limit FROM card JOIN card_fts ON (card.id = card_fts.id) " +
