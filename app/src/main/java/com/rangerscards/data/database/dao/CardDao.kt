...
interface CardDao {
    ...
    @Query("""SELECT * FROM (
            ...
            FROM card WHERE code = :cardCode AND (:taboo IS 1 AND taboo_id IS NOT NULL) AND pack_id IN (:packIds)
            UNION ALL
            ...
            SELECT taboo_id, aspect_id, aspect_short_name, cost, image_src, real_image_src, name,
                ...
                set_name, set_size, set_position, pack_short_name, subset_name, subset_position, subset_size,
                sun_challenge, mountain_challenge, crest_challenge
            FROM card AS c WHERE code = :cardCode AND (:taboo IS 1 AND taboo_id IS NULL) AND pack_id IN (:packIds)
              AND NOT EXISTS ( SELECT 1 FROM card c2 WHERE c2.code = c.code AND c2.taboo_id IS NOT NULL)
            UNION ALL
            ...
            SELECT taboo_id, aspect_id, aspect_short_name, cost, image_src, real_image_src, name,
                ...
                set_name, set_size, set_position, pack_short_name, subset_name, subset_position, subset_size,
                sun_challenge, mountain_challenge, crest_challenge
            FROM card WHERE code = :cardCode AND (:taboo IS 0 AND taboo_id IS NULL) AND pack_id IN (:packIds)
        )""")
